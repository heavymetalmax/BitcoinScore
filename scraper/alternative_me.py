    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            result = pg.evaluate("""() => {
                ...
                return {date: last[0] ? last[0].slice(0,10) : null, score: last[2], label: last[3]};
            }""")
            br.close()
        if result and result.get('score') is not None:
            score = int(result['score'])
            label = result.get('label') or _score_to_label(score)
            logger.info('get_fear_greed: %s score=%s label=%s (BMP live)', result.get('date'), score, label)
        ...
        try:
            ...
            if series:
                ...
                if score is not None:
                    ...
                    return {'value': int(score), 'label': label}
        except Exception as e:
