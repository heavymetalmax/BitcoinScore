...
def _call_openai(prompt: str, api_key: str) -> Optional[str]:
    ...
    developer_instruction = (
        ...
        "SENTENCE STRUCTURE:\n"
        "  S1: Synthesise the structural picture in one sentence — "
        "    combine the most informative on-chain signal with the active regime. "
        "    Use 1–2 metric values as evidence. Do NOT lead with the composite score.\n"
        "  S2: Assess the probability that the current index DIRECTION continues.\n"
        "    Step 1 — infer direction: if the dominant metrics (ETF flows, MVRV, aSOPR) "
        "    are in decline/capitulation territory, the index is falling; "
        "    if improving, it is rising; otherwise flat.\n"
        "    Step 2 — check driver momentum: are the factors driving that direction "
        "    still active and strong, or losing force?\n"
        "    Step 3 — assign probability:\n"
        "      HIGH continuation: primary driver is still strong and accelerating "
        "        (e.g. ETF outflows deepening, aSOPR still <1, no reversal signals).\n"
        "      MODERATE: driver is present but showing early signs of stalling "
        "        (e.g. ETF outflows steady but not widening, aSOPR near 1.00).\n"
        "      LOW: primary driver is weakening or a counter-signal is emerging.\n"
        "    Step 4 — name the single leading indicator whose REVERSAL would signal "
        "    the trend is about to change direction (not the current trend's driver).\n"
        "    IMPORTANT: if the trend is DOWNWARD and ETF outflows are the primary driver, "
        "    state 'high probability of further decline' — do NOT imply the trend is "
        "    'contingent on ETF inflows' as if inflows are the base case.\n\n"

        "STRICT RULES:\n"
        "  - Exactly 2 sentences. No bullet points, headers, or markdown.\n"
